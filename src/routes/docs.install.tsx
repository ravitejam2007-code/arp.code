...
export const Route = createFileRoute("/docs/install")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Install ARPCODE on macOS, Windows, and Linux. Step-by-step Node.js setup, package manager commands, PATH troubleshooting." },
  ...
  component: () => (
    ...
    >
      ...
      <ul className="list-disc pl-6 text-muted-foreground space-y-1">
        <li>Node.js <strong className="text-foreground">≥ 20.0.0</strong> (ESM runtime)</li>
        <li>A truecolor-capable terminal (iTerm2, WezTerm, Alacritty, Kitty, Windows Terminal 1.18+, GNOME Terminal)</li>
        <li>~10&nbsp;MB disk for the CLI itself, plus ~80&nbsp;MB for the global <Code>node_modules</Code></li>
        <li>Internet access (cloud providers) or a local Ollama install (offline mode)</li>
      </ul>

      <H2 id="macos">macOS</H2>
      <H3>1. Install Node.js</H3>
      <P>Recommended: install via <Code>nvm</Code> so you can switch versions later.</P>
...
node --version   # v20.x.x`}</CodeBlock>
      <P>Prefer Homebrew? <Code>brew install node@20</Code> works too.</P>
...
arpcode`}</CodeBlock>
      ...
      <Callout>
        On macOS with nvm, global npm packages install under your nvm Node prefix — no <Code>sudo</Code> needed.
      </Callout>
...
npm install -g arp-code`}</CodeBlock>
      ...
      <Callout kind="warn">
        Use <strong>Windows Terminal 1.18+</strong> for proper truecolor and Unicode rendering. The legacy
...
npm install -g arp-code`}</CodeBlock>
      ...
      <P>Configure npm to use a user-owned prefix so <Code>npm install -g</Code> never needs root.</P>
...
yarn global add arp-code`}</CodeBlock>
      <P>All four install the same two binaries: <Code>arp-code</Code> and the shorter alias <Code>arpcode</Code>.</P>
...
# Lists all flags and subcommands`}</CodeBlock>
      ...
      <H2>First run</H2>
      <P>
        The first launch opens a 4-step onboarding wizard: pick a default provider, paste an API key (or
        skip for Ollama), choose a theme, and you're in. Config is written to{" "}
        <Code>~/.arp-code/config.json</Code>.
      </P>
      <Callout>
        Prefer running locally? Install <a href="https://ollama.ai" className="text-primary underline-offset-4 hover:underline">Ollama</a> first
        — ARPCODE auto-discovers a server on <Code>http://127.0.0.1:11434</Code> and skips the API-key step entirely.
      </Callout>

      <H2>Troubleshooting</H2>
      <H3>`arpcode: command not found`</H3>
      <P>Your shell's <Code>PATH</Code> doesn't include npm's global bin directory. Run:</P>
...
# Windows:     append <prefix>     (it contains arpcode.cmd)`}</CodeBlock>
      ...
      <P>Switch to a terminal that supports truecolor and UTF-8 (Windows Terminal 1.18+, iTerm2, WezTerm, Alacritty).</P>
