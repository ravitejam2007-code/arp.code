...
export function H2({ children, id }: { children: ReactNode; id?: string }) {
  return (
    <h2 id={id} className="mt-12 font-mono text-2xl font-bold border-b border-border pb-2">
      {children}
    </h2>
  );
}
...
export function Code({ children }: { children: ReactNode }) {
  return (
    <code className="bg-card border border-border px-1.5 py-0.5 text-[13px] text-accent">
      {children}
    </code>
  );
}
...
export function CodeBlock({ children, lang }: { children: ReactNode; lang?: string }) {
  ...
  return (
    <div className="border border-border bg-card overflow-hidden group relative">
      ...
      <pre className="overflow-x-auto p-4 font-mono text-[13px] text-foreground leading-relaxed">
        <code>{Children.toArray(children)}</code>
      </pre>
    </div>
  );
}
export function Callout({
  ...
  const cls = kind === "warn" ? "border-amber bg-amber/10" : "border-primary bg-primary/5";
        ...
        <thead className="bg-card">
          <tr>
            {headers.map((h) => (
              <th
                key={h}
                className="text-left font-mono text-[12px] uppercase tracking-wider text-muted-foreground px-3 py-2 border-b border-border"
              >
                {h}
              </th>
            ))}
